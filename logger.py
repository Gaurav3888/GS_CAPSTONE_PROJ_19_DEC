...
def update_train_log(self,data_shape, eval_test, runtime, MODEL_VERSION, MODEL_VERSION_NOTE, test=False):
    ...
    today = date.today()
    if test:
        logfile = os.path.join("logs", "train-test_trial.log")
    else:
        logfile = os.path.join("logs", "training-{}-{}.log".format(today.year, today.month))
...
def update_predict_log(self,y_pred, y_proba, query, runtime, MODEL_VERSION, test=False):
    ...
    today = date.today()
    if test:
        logfile = os.path.join("logs", "prediction-test.log")
    else:
